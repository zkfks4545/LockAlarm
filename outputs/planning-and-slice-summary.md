...
- 여러 알람을 만들고 1회성 또는 요일 반복을 선택하며, 반복 일정에 추가·제외 날짜를 지정할 수 있다.
- 최초 울림에서 5분 스누즈를 한 번 사용할 수 있다.
- 울림 시작 시 밝기와 미디어 음량을 지정 초기값으로 한 번만 적용한다. 이후 사용자의 조작을 막거나 다시 덮어쓰지 않는다.
...
- 편집 화면: 이름, 선택 창 또는 숫자 직접 입력 시각, 오늘 지난 시각의 내일 자동 보정, 오늘·내일 빠른 날짜, 단발/요일 반복, 추가·제외 날짜, 다음 실행 미리보기.
...
- 편집 화면의 유효한 YouTube URL은 공식 썸네일과 재생 버튼을 제공하고, 탭 시 편집 화면 전용 공식 IFrame 미리보기와 닫기 동작으로 전환한다. 빈/지원 불가 URL은 안내하며 다운로드·캐시·MP3 추출은 사용하지 않는다.
...
- 요일 반복은 전달 직후 다음 유효 발생을 계산·저장·재등록한다.
...
- 현재 울림 중 도착한 정규 알람은 발생 시각 순으로 대기하고, 현재 알람 해제 직후 시작한다.
- 스누즈 대기 중 새 정규 알람은 기존 스누즈를 종료·복원한 뒤 우선 실행한다.
...
- 종료 지연 중 영상 입력·X·위 스와이프·스누즈는 전체화면 차단막으로 막고, 지연 완료 뒤 차단막 제거와 하단 좌측 1회 스누즈·우측 큰 X·160dp 위 스와이프를 함께 활성화한다.
...
- YouTube 모드는 앱 ID 기반 HTTPS Referer·origin으로 공식 임베디드 플레이어를 로드하고 임베드 금지·비공개·식별 실패를 구분해 안내한다.
...
- 메인 알람 목록은 좁은 화면 세로 카드·하단 플로팅 내비게이션과 넓은 화면 좌측 레일·2열 카드로 표시하며, 상단에서 저장 가능한 다크·라이트 테마를 전환한다.
...
- JVM 단위 테스트 72건, 실패/오류 0.
- Android lint 오류 0건. 경고 29건은 의존성 업데이트 안내, 구형 Android 썸네일 호환 API, 동기 세션 저장 의도, KAPT/KTX 제안 등이다.
- debug APK 조립 성공.
- 통합 debug APK: `outputs/routine-alarm-integrated-final-v11-debug.apk` (`routine-alarm-integrated-final-debug.apk`도 같은 파일로 교체).
- debug APK SHA-256: `2D3FCF7507B1D7600C768536507B35C0558807ADE1CEABB499ED6DC44E9581CD`.
...
- 자동화 검증은 `HH:mm` 포맷, 타이머 잠금/완료, 160dp 위 스와이프와 짧거나 수평 드래그 거부, 최초 1회 스누즈와 기존 세션 유지까지 포함한다.
