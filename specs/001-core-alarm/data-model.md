...
- Room `alarm_occurrences`: 정규 발생의 revision/occurrence/session 식별자와 `CLAIMED`, `WAITING`, `FIRING`, `SNOOZED`, 종료 상태를 저장한다. 전달 수락과 겹침 조정은 이 테이블을 사용하는 단일 Room 트랜잭션에서 선점한다.
...
| `ruleType` | `ONE_TIME` 또는 `WEEKLY` |
| `oneTimeDate` | 1회성 날짜, WEEKLY에서는 없음 |
| `weekdays` | WEEKLY의 1개 이상 선택 요일 |
...
| `dismissDelaySeconds` | 우측 상단 원형 진행이 종료 조작으로 전환될 때까지의 지연(켜짐 상태에서 0~60초) |
...
검증 규칙: ONE_TIME은 미래 또는 정책상 허용된 날짜 하나를 가져야 한다. WEEKLY는 요일이 하나 이상이어야 한다. 같은 날짜가 포함·제외 양쪽에 있으면 저장 전에 충돌을 표시하며 실행 계산에서는 제외가 우선한다.
...
스누즈는 새 세션을 만들지 않는다. 재울림에서는 `ringStartedAt`과 `dismissAvailableAt`만 갱신하고 `previousDeviceState`, `presetApplied`, `snoozeCount`를 유지한다.

겹침 시 `FIRING`/`CLAIMED`가 있으면 새 정규 발생은 `WAITING`으로 선점한다. 현재 발생이 `SNOOZED`이면 이를 `PREEMPTED`로 전이하고 새 정규 발생을 `CLAIMED`로 선점한다. `SNOOZE_CLAIMED`는 이미 재울림이 시작된 것으로 보아 새 정규 발생이 대기한다.
